...
export const Catalog: React.FC = () => {
  ...
  const fetchCatalogData = async () => {
    setLoading(true);
    try {
      const plData = await api.get<PriceList[]>('/inventory/pricelists/');
      setPricelists(plData);
      
      const query = selectedPricelist ? `?pricelist_id=${selectedPricelist}` : '';
      const prodData = await api.get<Product[]>(`/inventory/products/${query}`);
          ...
          {filteredProducts.map(product => {
            ...
            return (
              <div key={product.id} className="glass-panel glass-panel-hover" style={{
                ...
                {/* Product Image placeholder or generated image */}
                <div style={{
                  height: '180px',
