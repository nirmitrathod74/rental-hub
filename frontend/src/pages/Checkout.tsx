...
export const Checkout: React.FC = () => {
  ...
  const [successOrder, setSuccessOrder] = useState<RentalOrder | null>(null);
  ...
  const handlePaySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const order = await api.post<RentalOrder>('/rentals/orders/', orderPayload);

      // 2. Confirm order and collect payment
      const confirmPayload = {
        amount_paid: finalRent + finalDelivery,
        deposit_paid: totalDeposit
      };

      const confirmedOrder = await api.post<RentalOrder>(`/rentals/orders/${order.id}/confirm/`, confirmPayload);
      setSuccessOrder(confirmedOrder);
      clearCart();

      // Get invoice download URL
      try {
        const invRes = await api.get<{ invoice_url: string }>(`/rentals/orders/${order.id}/invoice/`);
  ...
  return (
    <div className="fade-in" style={{ padding: '32px', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      <div style={{
        ...
        {/* Left - Payment simulated Card Form */}
        ...
        {/* Right - Order invoice breakdown summary */}
